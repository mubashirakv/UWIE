...
def process():
    ...
    if file:
        ...
        if request.form.get('file_type') == 'image':
            output_path = f'static/{filename}'
            mat = cv2.imread(file_path)
            mat = cv2.cvtColor(mat, cv2.COLOR_BGR2RGB)
            corrected_mat = correct(mat)
            cv2.imwrite(output_path, corrected_mat)
        elif request.form.get('file_type') == 'video':
            output_path = f'static/{filename.split(".")[0]}.mp4'
            for item in analyze_video(file_path, output_path):
                if type(item) == dict:
                    video_data = item
